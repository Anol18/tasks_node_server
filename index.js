...
app.get("/dashboard", (req, res) => {
  ...
  // Define all possible statuses and priorities
  ...
  // Group tasks by status (ensure all statuses are included)
  const statusSummary = allStatuses.reduce((acc, status) => {
    acc[status] = tasks.filter((task) => task.status === status).length;
    return acc;
  }, {});

  // Group tasks by priority (ensure all priorities are included)
  const prioritySummary = allPriorities.reduce((acc, priority) => {
